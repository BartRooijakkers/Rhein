...
INNER JOIN afdelingen ON gebruikers.afdeling = afdelingen.afdeling_ID WHERE gebruikers.account_status = 0 GROUP BY gebruikers.gebruiker_ID ORDER BY achter_naam DESC")
?>
        ...
        <div class="contentTileAuto">
            <?php   if($sql_requests->rowCount()> 0){
                ...
                while ($row = $sql_requests->fetch()) {
                    ...
                    <td><a class='button green' href='userList.php?request_status=1&id={$row->gebruiker_ID}'>Accept</a> <a class='button red' href='userList.php?request_status=2&id={$row->gebruiker_ID}'>Deny</a></td></tr></table>";
                }
            }else{
